...
{
    private async Task UserTableExportAsync(Request req, CancellationToken cancellationToken)
    {
        string guid = Guid.NewGuid().ToString(); //lets generate guid
        req.Guid = guid;

        //set state of request to processing
        req.IdState = await _context.RequestStates
            .DictionaryStateAsync(eRequestState.Processing, cancellationToken);
        ...
        try
        {
            ...
            req.IdState = await _context.RequestStates
                .DictionaryStateAsync(eRequestState.Ready, cancellationToken);
        }
        catch (Exception err)
        {
            ...
            req.IdState = await _context.RequestStates
                .DictionaryStateAsync(eRequestState.Error, cancellationToken);
        }
